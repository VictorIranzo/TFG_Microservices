...
Las arquitecturas basadas en microservicios son una tendencia actual asociada a conceptos como el desarrollo centrado en el dominio del problema o el despliegue en contenedores. En estas arquitecturas, una aplicación software se compone de servicios pequeños y autónomos que colaboran entre ellos.

El objetivo de este trabajo es validar con un caso de estudio las ventajas e inconvenientes de una arquitectura basada en microservicios frente a una arquitectura tradicional o monolítica. Se hará una revisión de la influencia de los microservicios en el proceso de desarrollo de software y se repasarán las principales herramientas asociadas a su despliegue. 

Posteriormente, se diseñará e implementará una misma aplicación móvil de comercio electrónico (e-shop) empleando en su parte servidora dos arquitecturas diferentes: una basada en microservicios y otra monolítica. Para su despliegue se empleará Microsoft Azure a través de contenedores Docker orquestados por la herramienta Kubernetes. 

Finalmente, ambas soluciones se compararán frente a diferentes requisitos no funcionales, como la disponibilidad o la tolerancia a fallos, y distintas situaciones de mantenimiento para extraer conclusiones.
...
El éxito en el uso de los microservicios está ligado a principios como la división de responsabilidades en el código, la organización de los equipos de trabajo o una correcta metodología en el proceso de desarrollo y no tanto al uso de una tecnología específica. Existen un gran número de herramientas que se pueden emplear en su desarrollo. Su uso facilita alcanzar requisitos no funcionales como los citados en la sección \ref{sect:RNF}. Sin embargo, las diferencias entre herramientas son mínimas, como prueba el gran número de funcionalidades similares que ofrecen AWS y Azure, por lo que no tiene relevancia emplear una u otra. 
...
\begin{figure}[h]
\centering
\includegraphics[scale=0.5]{ActiveDirectory} \label{fig:ActiveDirectory}
...
\begin{figure}[h]
\centering
\includegraphics[scale=0.5]{CreateCluster} \label{fig:CreateCluster}
...
En primer lugar, para acceder al clúster que se ha creado en el paso anterior debemos ejecutar el comando que se ilustra en la figura \ref{AccessCluster}, que recibe como parámetro el nombre del clúster creado y el grupo de recursos donde se encuentra. Una vez ejecutado, podemos comprobar la conexión con el clúster mediante un comando de Kubernetes, como el que se emplea para obtener los nodos del clúster. En nuestro caso, solo hemos creado un nodo en el clúster.

\begin{figure}[h]
\centering
\includegraphics[scale=0.6]{AccessCluster} \label{fig:AccessCluster}
...
\begin{figure}[h]
\centering
\includegraphics[scale=0.6]{KubernetesSecurity} \label{fig:KubernetesSecurity}
...
\begin{figure}[h]
\centering
\includegraphics[scale=0.8]{KubernetesApply} \label{fig:KubernetesApply}
...
\begin{figure}[h]
\centering
\includegraphics[scale=0.55]{ServiciosKubernetes} \label{fig:ServiciosKubernetes}
\caption{Creación de los servicios en el clúster de Kubernetes.}
\end{figure}
...
\begin{figure}[h]
\centering
\includegraphics[scale=0.5]{MicroservicioDesplegado} \label{fig:MicroservicioDesplegado}
...
\begin{figure}[h]
\centering
\includegraphics[scale=0.7]{appsettings} \label{fig:appsettings}
...
\begin{figure}[h]
\centering
\includegraphics[scale=0.65]{DespliegueProduccion} \label{fig:DespliegueProduccion}
...
\begin{figure}[h]
\centering
\includegraphics[scale=0.65]{ClusterIP} \label{fig:ClusterIP}
...
\begin{figure}[h]
\centering
\includegraphics[scale=0.7]{Ingress} \label{fig:Ingress}

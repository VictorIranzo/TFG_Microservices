...
%TODO Ver si añadir por qué es bueno usar interfaces.
...
\item En la capa de persistencia, para asegurar la creación de las tablas asociadas a Identity, el contexto de la aplicación que representa a la base de datos debe extender la clase IdentityDbContext<User>.
...
\begin{figure}[h]
\centering
\includegraphics[scale=0.8]{Authorize}
\caption{Ejemplo de métodos de la clase de servicios.}
\end{figure}

\item Por último, en la capa de servicios se debe especificar que el mecanismo por el cual el usuario proveerá su identidad será a través de tokens JWT.
...
\subsection{Documentando la API con Swagger}

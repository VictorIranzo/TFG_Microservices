...
\vspace{2mm}
\tabitem \textbf{Escalabilidad}: en las arquitecturas monolíticas, para escalar una funcionalidad concreta hace falta escalar el sistema completo. Empleando microservicios, se puede escalar cada servicio de forma distinta según sus necesidades. Esto produce un mejor aprovechamiento de los recursos y un ahorro en los costes \cite{Newman2015a, DelaTorre2018, Lewis2014}.

\vspace{2mm}
\tabitem \textbf{Facilidad para evolucionar}: un cambio en una línea de código de una aplicación monolítica supone el despliegue del sistema completo para publicar dicho cambio. El riesgo que supone el despliegue es mayor porque comprende una mayor base de código. Empleando microservicios, un cambio en un único servicio se despliega de manera independiente, haciendo que los cambios lleguen al cliente final de forma más rápida \cite{Newman2015a, DelaTorre2018, Lewis2014}.

\vspace{2mm}
\tabitem \textbf{Programación y persistencia políglota}: se puede escoger una tecnología diferente para cada uno de los servicios. De esta manera, se escoge aquella que mejor se adecue a las necesidades concretas del servicio y no una tecnología estándar válida para todos ellos. Esto se aplica tanto al lenguaje de programación empleado, la arquitectura del servicio o la tecnología de base de datos empleada \cite{Newman2015a, DelaTorre2018, Lewis2014}.

\vspace{2mm}
\tabitem \textbf{Facilidad para reemplazar}: el sistema se divide en componentes que pueden ser reescritos en pocos días. Gracias al uso de contratos, basta con seguir ofreciendo la misma interfaz para que ningún consumidor se rompa \cite{Newman2015a, Lewis2014}.

&

\vspace{2mm}
\tabitem \textbf{Depuración}: encontrar un defecto es sencillo si la funcionalidad que se ve afectada no involucra a más de un servicio. Sin embargo, si más de un servicio es partícipe, depurar el caso de uso al completo requiere una mayor comprensión y un uso de recursos superior.

\vspace{2mm}
\tabitem \textbf{Consistencia eventual}: no se pueden realizar transacciones como se realiza en la base de datos monolítica. Se tienen que implementar mecanismos de compensación para que, en caso de que parte de la antigua transacción falle en uno de los microservicios, se restauren los valores modificados en el resto. La integridad referencial no se puede realizar a través de claves ajenas porque existe más de una base de datos implicada, lo que significa que se tienen que implementar mecanismos para asegurar la integridad de los datos \cite{Newman2015a, DelaTorre2018, Lewis2014}.

\vspace{2mm}
\tabitem \textbf{Código duplicado}:

\vspace{2mm}
\tabitem \textbf{Mayores necesidades de recursos}: \cite{DelaTorre2018}

\vspace{2mm}
\tabitem \textbf{Incremento de la deuda técnica}: cuando se desarrolla un sistema completo empleando microservicios, para finalizar el desarrollo de un microservicio completo se pueden tomar atajos. Estos atajos representan trabajo que se debe realizar en un futuro por no haber implementado la mejor solución posible \cite{Newman2015a, FowlerSusan}.
